= myFlicks

myFlicks is an online movie discussion board that fetches flick details from a third party IMDB API at imdbapi.com. It also uses the imgur.com API to host the posters since IMDB does not allow hotlinking images from their server.

== Features

* Fetch any flick by title or IMDB iid.
* View your flicks
* Comment on flicks
* Search on myFlicks internal database for flicks by title or year
* Quick-add button to add other user's flick to your list
* Authentication system based on Devsie gem
* First user to sign up is the admin. 'rake db:seed' after creating the first user to grant it admin rights.


== Upcoming features

* View other user's profile and flicks list
* Flick's show page will include a link to the trailer


== Known Bugs

* Flicks returned by internal search displays the quick add button even if the user has it in the list.

== Requirements:

* Ruby 2.0 w/ Rails 4.0

* Imgur.com API Key - https://api.imgur.com/#register

* omdbapi v2.1 - http://www.omdbapi.com/ 

* Gems: Devise and Bootstrap-sass


Part of CINS 465 - Project 8: 

Provide user authentication (you may use devise, another gem, or implement authentication from scratch).
Be complex enough to require a database with multiple tables that relate to each other (one-to-many or one-to-one).
Provide a service and/or features more complicated then the previous assignments.
Demonstrate your application to the class the last two weeks of the semester.

== Screenshots

http://i.imgur.com/JvbsHnS.png

http://i.imgur.com/dEfDudi.png

http://i.imgur.com/3X2pNPB.png

http://i.imgur.com/mJv5zoA.png

http://i.imgur.com/HlDeNpr.png
